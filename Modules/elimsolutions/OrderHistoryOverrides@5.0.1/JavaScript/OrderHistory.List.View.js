	...
	, function (
		...
		return Backbone.View.extend({
			...
			, sortOptions: [
				...
				, {
					value: 'tranid'
					, name: _('Sort by eS+ Order No.').translate()
				}
				/* ,	{
						value: 'amount'
						...
						, records_collection = new Backbone.Collection(this.collection.map(function (order) {
							...
							if (self.isSCISIntegrationEnabled) {
								...
								dynamic_column = {
									...
									, type: 'status'
									, name: 'status'
									, value: order.get('status').name === 'Pending Billing' ? 'Shipment Complete' : order.get('status').name
								};
							}
							...
							var model = new Backbone.Model({
								...
								, customerOrder: order.get('otherrefnum')
								, columns: columns
							});
